    ...
    <div style={{ minHeight: '100vh', paddingBottom: 96, background: 'var(--bg)' }}>
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        <div className="max-w-lg mx-auto px-4 py-4">
          ...
          <h1 className="text-xl font-bold" style={{ color: 'var(--charcoal)' }}>Catalogs</h1>
          <p className="text-xs" style={{ color: 'var(--muted)' }}>Products, techniques, and tools tracked by Labs</p>
        </div>
        ...
        <div className="max-w-lg mx-auto px-4" style={{ display: 'flex', gap: 0, borderTop: '1px solid var(--border)', overflowX: 'auto' }}>
          {tabs.map((t) => {
            ...
            return (
              ...
              >
                {t.label}
                <span style={{ fontSize: 10, color: active ? LABS_COLOR : 'var(--muted)', opacity: 0.8 }}>({t.count})</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="max-w-lg mx-auto px-4 mt-4">
        {isLoading ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm" style={{ color: 'var(--muted)' }}>Loading catalog...</p>
          </div>
        ) : (
          <div className="space-y-2">
            {tab === 'products' && products.map((p) => (
              <div key={p.id} className="p-3" style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card)' }}>
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-semibold" style={{ color: 'var(--charcoal)' }}>{p.name}</h3>
                    <p className="text-xs" style={{ color: 'var(--muted)' }}>{p.category}{p.brand ? ` · ${p.brand}` : ''}</p>
                  </div>
                  {p.tags && p.tags.length > 0 && (
                    <div className="flex gap-1">
                      {p.tags.slice(0, 2).map((tag) => (
                        <span key={tag} className="px-2 py-0.5 text-xs rounded-full" style={{ background: 'var(--surface-2)', color: 'var(--mid)' }}>{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
            ...
            {tab === 'techniques' && techniques.map((t) => (
              <div key={t.id} className="p-3" style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card)' }}>
                <h3 className="text-sm font-semibold" style={{ color: 'var(--charcoal)' }}>{t.name}</h3>
                <p className="text-xs" style={{ color: 'var(--muted)' }}>{t.category}</p>
                {t.description && (
                  <p className="text-xs mt-1 line-clamp-2" style={{ color: 'var(--muted)' }}>{t.description}</p>
                )}
              </div>
            ))}
            ...
            {tab === 'tools' && toolMethods.map((t) => (
              <div key={t.id} className="p-3" style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card)' }}>
                <h3 className="text-sm font-semibold" style={{ color: 'var(--charcoal)' }}>{t.name}</h3>
                <p className="text-xs" style={{ color: 'var(--muted)' }}>{t.toolType}{t.brand ? ` · ${t.brand}` : ''}</p>
                {t.specification && (
                  <p className="text-xs mt-1" style={{ color: 'var(--muted)' }}>{t.specification}</p>
                )}
              </div>
            ))}
            ...
            {tab === 'products' && products.length === 0 && (
              <div className="text-center py-8">
                <p className="text-sm" style={{ color: 'var(--muted)' }}>No products in catalog</p>
              </div>
            )}
            {tab === 'techniques' && techniques.length === 0 && (
              <div className="text-center py-8">
                <p className="text-sm" style={{ color: 'var(--muted)' }}>No techniques in catalog</p>
              </div>
            )}
            {tab === 'tools' && toolMethods.length === 0 && (
              <div className="text-center py-8">
                <p className="text-sm" style={{ color: 'var(--muted)' }}>No tools in catalog</p>
              </div>
            )}
          </div>
        )}
