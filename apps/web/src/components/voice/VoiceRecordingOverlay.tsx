...
export function VoiceRecordingOverlay({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[var(--charcoal)]/95 animate-in fade-in duration-200">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-4 bg-[var(--charcoal)]/50">
        <button
          onClick={onCancel}
          className="min-w-[48px] min-h-[48px] flex items-center justify-center text-[var(--muted)] hover:text-white transition-colors"
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[var(--muted)] font-mono">{formatDuration(duration)}</span>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-center gap-1">
            {[0, 1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="w-1.5 h-8 bg-[var(--accent)] rounded-full animate-pulse"
                style={{
          ...
          <p className="text-[var(--muted)] text-sm mt-4 text-center">Listening...</p>
        </div>
        ...
        <div className="max-w-md w-full">
          {displayText ? (
            <p className="text-white text-xl leading-relaxed text-center">
              {transcript}
              {interimTranscript && (
                <span className="text-[var(--muted)]">{` ${interimTranscript}`}</span>
              )}
            </p>
          ) : (
            <p className="text-[var(--muted)] text-lg text-center italic">
              Speak now...
            </p>
          )}
        </div>
