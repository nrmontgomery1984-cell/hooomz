    ...
    return (
      <div style={{ minHeight: '100vh', paddingBottom: 96, background: 'var(--bg)' }}>
        <div className="max-w-lg mx-auto px-4 py-12 text-center">
          ...
          <p className="text-sm" style={{ color: 'var(--muted)' }}>Loading voting...</p>
        </div>
      </div>
    );
  }
    ...
    <div style={{ minHeight: '100vh', paddingBottom: 96, background: 'var(--bg)' }}>
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        <div className="max-w-lg mx-auto px-4 py-4">
          ...
          <h1 className="text-xl font-bold" style={{ color: 'var(--charcoal)' }}>Partner Voting</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>Weekly research priority ballots</p>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 mt-4 space-y-4">
        ...
        {activeBallot ? (
          <div>
            <h2 className="text-sm font-semibold mb-2 px-1" style={{ color: 'var(--mid)' }}>Active Ballot</h2>
            <VoteBallotCard
              ballot={activeBallot}
              hasVoted={hasVoted}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 24, textAlign: 'center', boxShadow: 'var(--shadow-card)' }}>
            <p className="text-sm" style={{ color: 'var(--muted)' }}>No active ballot this week.</p>
          </div>
        )}
        ...
        {closedBallots.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold mb-2 px-1" style={{ color: 'var(--mid)' }}>Past Ballots</h2>
            <div className="space-y-3">
              {closedBallots.map((ballot) => (
                <div key={ballot.id} style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 16, boxShadow: 'var(--shadow-card)' }}>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <h3 className="text-sm font-semibold" style={{ color: 'var(--charcoal)' }}>Week {ballot.id}</h3>
                      <p className="text-xs" style={{ color: 'var(--muted)' }}>{ballot.weekStart} — {ballot.weekEnd}</p>
                    </div>
                    <span className="text-xs" style={{ color: 'var(--muted)' }}>{ballot.totalVotes} votes</span>
                  </div>
                  <VoteResultsChart
        ...
        {allBallots.length === 0 && (
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 32, textAlign: 'center', boxShadow: 'var(--shadow-card)' }}>
            <p className="text-sm" style={{ color: 'var(--muted)' }}>No ballots yet.</p>
