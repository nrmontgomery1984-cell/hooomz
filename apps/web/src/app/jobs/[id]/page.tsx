...
const TRC: Record<string, { c: string; b: string; bg: string }> = {
  paint: { c: '#F59E0B', b: 'rgba(245,158,11,.25)', bg: 'rgba(245,158,11,.08)' },
  floor: { c: '#14B8A6', b: 'rgba(20,184,166,.25)', bg: 'rgba(20,184,166,.08)' },
  trim:  { c: '#A855F7', b: 'rgba(168,85,247,.25)', bg: 'rgba(168,85,247,.08)' },
  doors: { c: '#F97316', b: 'rgba(249,115,22,.25)', bg: 'rgba(249,115,22,.08)' },
};

const TGC: Record<string, { c: string; b: string; bg: string }> = {
  note:  { c: '#A3B3FF', b: 'rgba(26,86,219,.25)',  bg: 'rgba(26,86,219,.08)' },
  warn:  { c: '#F59E0B', b: 'rgba(245,158,11,.3)',  bg: 'rgba(245,158,11,.08)' },
  block: { c: '#EF4444', b: 'rgba(239,68,68,.25)',  bg: 'rgba(239,68,68,.07)' },
...
const SEED_ROOMS: Record<string, Room> = {
  living: {
    ...
    cm: [{ a: 'Nishant', av: 'N', ac: '#14B8A6', tag: 'note', t: 'Feb 10\u00b72pm', b: 'Ceilings and walls done. Two coats. Looks great.' }],
  },
  kitchen: {
    ...
    cm: [
      { a: 'Nathan', av: 'NM', ac: '#1A56DB', tag: 'warn', t: 'Feb 11\u00b78:30am', b: 'Check grout color in daylight \u2014 looked different under artificial light.' },
      { a: 'Homeowner', av: 'HO', ac: '#F59E0B', tag: 'owner', t: 'Feb 11\u00b79:10am', b: 'Lighter grout is fine with me if it looks better!' },
    ],
  },
  bath: {
    ...
    cm: [{ a: 'Nishant', av: 'N', ac: '#14B8A6', tag: 'block', t: 'Feb 11\u00b711am', b: 'Tile delivery delayed to Feb 13. All bath tasks blocked.', ph: 'bath-receipt.jpg' }],
  },
  bed1: {
    ...
    cm: [
      { a: 'Nishant', av: 'N', ac: '#14B8A6', tag: 'note', t: 'Feb 11\u00b79:42am', b: 'Sub-floor soft near window wall \u2014 checking before LVP.', ph: 'subfloor-bed1.jpg' },
      { a: 'Nathan', av: 'NM', ac: '#1A56DB', tag: 'warn', t: 'Feb 11\u00b710:15am', b: 'Check moisture before proceeding. Over 12% = pause and raise CO.' },
    ],
  },
  bed2: {
...
const TRADE_FILTERS: Array<[string, string]> = [
  ['All', '#1A56DB'], ['Paint', '#F59E0B'], ['Floor', '#14B8A6'], ['Trim', '#A855F7'],
];

const LEGEND: Array<[string, string]> = [
  ['#22C55E', 'Complete'], ['#1A56DB', 'Active'], ['#EF4444', 'Blocked'], ['#525868', 'Pending'],
];
...
const TEAM = [
  { name: 'Nathan M.', initials: 'NM', color: '#1A56DB', role: 'Lead / PM', hours: '4h' },
  { name: 'Nishant', initials: 'N', color: '#14B8A6', role: 'Installer', hours: '7h' },
];
...
function RoomDetail({ room, onClose }: { room: Room; onClose: () => void }) {
  ...
  const stO: Record<string, string> = {
    done: 'rgba(34,197,94,.06)', active: 'rgba(26,86,219,.09)',
    blocked: 'rgba(239,68,68,.11)', pending: 'rgba(0,0,0,0)',
  };
    ...
    <div style={{
      ...
      <div style={{ padding: '12px 13px 10px', borderBottom: '1px solid var(--border)', background: 'var(--surface-2)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 3 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: 'var(--font-cond)', fontSize: 18, fontWeight: 800, letterSpacing: '.06em', lineHeight: 1.1, color: 'var(--text)' }}>{room.name}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)' }}>{parts}</span>
          ...
          <div onClick={onClose} style={{ width: 24, height: 24, borderRadius: '50%', background: 'var(--surface-3)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 12, color: 'var(--text-3)', flexShrink: 0, marginTop: 1 }}>\u2715</div>
        </div>
      ...
      <div style={{ padding: '12px 12px 6px', background: 'var(--bg)', flexShrink: 0 }}>
        <svg viewBox={`0 0 ${sw} ${sh}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
          ...
          <text fontFamily="'IBM Plex Mono',monospace" fontSize={7.5} fill={stC} textAnchor="middle" x={rx + rw / 2} y={ry + rh / 2 + 3}>{room.prog}%</text>
        </svg>
      </div>

      {/* Task list */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px 12px' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', marginBottom: 6 }}>Tasks</div>
        {room.tl.map((t, i) => {
          const done = t.s === 'done', act = t.s === 'active', blk = t.s === 'blocked';
          const tc = TRC[t.tr] ?? TRC.trim;
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '5px 8px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 4, marginBottom: 3, opacity: done ? .55 : 1 }}>
              <div style={{ width: 12, height: 12, borderRadius: 3, border: `1.5px solid ${done ? '#22C55E' : act ? '#1A56DB' : 'var(--text-3)'}`, background: done ? '#22C55E' : act ? 'rgba(26,86,219,.15)' : 'var(--surface-3)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, color: done ? '#fff' : '', fontWeight: 700 }}>{done ? '\u2713' : blk ? '!' : ''}</div>
              <div style={{ flex: 1, fontSize: 11, fontWeight: 500, color: done ? 'var(--text-3)' : 'var(--text)' }}>{t.n}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--text-3)' }}>{t.h}</span>
              </div>
            </div>
          );
        })}
        {latest && (
          <div style={{ marginTop: 10, padding: '8px 9px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 4 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 7, textTransform: 'uppercase', color: 'var(--text-3)', letterSpacing: '.08em', marginBottom: 5 }}>Latest Note</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text)' }}>{latest.a}</span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--text-3)', marginLeft: 'auto' }}>{latest.t}</span>
            </div>
            <div style={{ fontSize: 11, color: 'var(--text-2)', lineHeight: 1.5 }}>{latest.b}</div>
          </div>
        )}
      </div>
    </div>
...
export default function JobDetailPage() {
  ...
  function post() {
    if (!note.trim()) return;
    setRooms(p => ({ ...p, [cur]: { ...p[cur], cm: [...p[cur].cm, { a: 'Nathan', av: 'NM', ac: '#1A56DB', tag: 'note', t: 'Just now', b: note.trim() }] } }));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg)', fontFamily: 'var(--font-sans)', color: 'var(--text)', overflow: 'hidden' }}>
      <style>{`
      ...
      {/* TOPBAR                                                           */}
      {/* ================================================================ */}
      <div style={{ height: 44, background: 'var(--surface-1)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', padding: '0 14px', gap: 10, flexShrink: 0 }}>
        <div style={{ fontFamily: 'var(--font-cond)', fontSize: 17, fontWeight: 800, letterSpacing: '.08em', color: 'var(--text)' }}>
          HOO<span style={{ color: 'var(--blue)' }}>O</span>MZ
        </div>
        <span style={{ color: 'var(--text-3)' }}>\u203a</span>
        <div>
          <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)' }}>Bradley Rental Refresh</div>
          <div style={{ fontSize: 10, color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>12 Queen St, Moncton \u00b7 Job #2025-014</div>
        </div>
          ...
          {SCRIPT_STAGES.map(([n, s]) => (
            <div key={n} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
              <div style={{
                ...
                borderColor: s === 'done' ? '#22C55E' : s === 'active' ? '#1A56DB' : 'var(--text-3)',
                background: s === 'done' ? 'rgba(34,197,94,.15)' : s === 'active' ? 'rgba(26,86,219,.15)' : 'var(--surface-3)',
                boxShadow: s === 'active' ? '0 0 6px rgba(26,86,219,.4)' : 'none',
              }} />
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 6, color: 'var(--text-3)', textTransform: 'uppercase' }}>{n}</span>
            </div>
          ))}
        </div>

        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, padding: '3px 9px', borderRadius: 3, border: '1px solid var(--blue-dim)', background: 'var(--blue-dim)', color: 'var(--blue)', textTransform: 'uppercase' }}>Install Phase</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-3)' }}>Mar 6 \u00b7 2:14 PM</span>
          <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'var(--blue-dim)', border: '1px solid rgba(26,86,219,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'var(--blue)' }}>NM</div>
        </div>
      </div>
      ...
      <div style={{ height: 38, background: 'var(--surface-2)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', padding: '0 14px', gap: 6, flexShrink: 0 }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.08em', marginRight: 4 }}>Trade</span>
        {TRADE_FILTERS.map(([l, c]) => (
          <div key={l} style={{ fontFamily: 'var(--font-mono)', fontSize: 9, padding: '3px 9px', borderRadius: 3, border: '1px solid var(--border)', cursor: 'pointer', color: 'var(--text-2)', background: 'var(--surface-3)', textTransform: 'uppercase', borderLeft: `2px solid ${c}` }}>{l}</div>
        ))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 4 }}>
          <div style={{ position: 'relative', width: 32, height: 32 }}>
            <ProgressRing pct={overallProg} color="#1A56DB" size={32} stroke={2.5} />
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-2)' }}>{doneTasks}/{totalTasks} tasks</div>
          {totalFlags > 0 && (
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
          {LEGEND.map(([c, l]) => (
            <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--text-3)', textTransform: 'uppercase' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: 'var(--bg)' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 36, transform: `translateX(${planTX}) scale(${planSc})`, transformOrigin: 'center left', transition: 'transform .28s ease', zIndex: 1 }}>
            <svg viewBox="0 0 600 420" style={{ width: '100%', maxWidth: 670, height: 'auto', filter: 'drop-shadow(0 16px 48px rgba(0,0,0,.35))' }}>
              ...
              {Object.entries(rooms).map(([id, rm]) => {
                ...
                return (
                  <g key={`lbl-${id}`} style={{ pointerEvents: 'none' }}>
                    <text fontFamily="'Barlow Condensed',sans-serif" fontSize={isSel ? 11 : 9} fontWeight={700} fill={isSel ? 'rgba(128,128,128,.9)' : 'rgba(128,128,128,.5)'} textAnchor="middle" letterSpacing=".06em" x={cx} y={cy - 8}>{rm.name.toUpperCase()}</text>
                    <text fontFamily="'IBM Plex Mono',monospace" fontSize={6.5} fill={isSel ? 'rgba(128,128,128,.5)' : 'rgba(128,128,128,.25)'} textAnchor="middle" x={cx} y={cy + 3}>{rm.dim.split('\u00b7')[0].trim()}</text>
                    <circle cx={cx - 7} cy={cy + 13} r={2.2} fill={SCH[rm.st]} opacity={.75} />
                    <text fontFamily="'IBM Plex Mono',monospace" fontSize={6.5} fill={SCH[rm.st]} fillOpacity={.7} x={cx - 3} y={cy + 16}>{rm.prog}%</text>
                  </g>
                );
              })}
              ...
              <text fontFamily="'IBM Plex Mono',monospace" fontSize={7.5} fill="rgba(128,128,128,.35)" textAnchor="middle" x={283} y={408}>38.5\u2032</text>

              {/* Pins */}
              {PINS.map(([id, px, py, type, lbl]) => {
                const fc: Record<string, string> = { done: '#22C55E', warn: '#F59E0B', block: '#EF4444', note: '#1A56DB' };
                ...
                return (
                  <g key={`${id}-${type}`} onClick={e => { e.stopPropagation(); openPin(id); }} style={{ cursor: 'pointer', filter: 'drop-shadow(0 2px 5px rgba(0,0,0,.35))' }} transform={`translate(${px},${py})`}>
                    ...
                    <text fontFamily="'IBM Plex Mono',monospace" fontSize={6.5} fill="#fff" textAnchor="middle" y={2.5} fontWeight={700}>{lbl}</text>
                  </g>
                );
              })}
              ...
              <g transform="translate(555,394)">
                ...
                <text fontFamily="'IBM Plex Mono',monospace" fontSize={5} fill="rgba(128,128,128,.4)" textAnchor="middle" y={9}>N</text>
              </g>
            </svg>
          </div>
          ...
          {hover && !detail && rooms[hover] && (
            <div style={{ position: 'absolute', left: hpos.x + 16, top: hpos.y - 62, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, padding: '8px 11px', boxShadow: 'var(--shadow-panel)', pointerEvents: 'none', zIndex: 15, minWidth: 150 }}>
              <div style={{ fontFamily: 'var(--font-cond)', fontSize: 13, fontWeight: 700, marginBottom: 2, color: 'var(--text)' }}>{rooms[hover].name}</div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--text-3)', marginBottom: 4 }}>{rooms[hover].dim.split('\u00b7')[0].trim()}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              <div style={{ marginTop: 5, fontFamily: 'var(--font-mono)', fontSize: 7, color: 'var(--text-3)', borderTop: '1px solid var(--border)', paddingTop: 4 }}>Click to expand \u203a</div>
            </div>
          )}
          ...
          <div style={{ position: 'absolute', bottom: 14, left: '50%', transform: 'translateX(-50%)', display: 'flex', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, overflow: 'hidden', zIndex: 30 }}>
            {([
              ...
              <div key={i} onClick={fn || undefined} style={{ fontFamily: 'var(--font-mono)', fontSize: i === 1 ? 9 : 11, padding: i === 1 ? '5px 10px' : '5px 13px', cursor: fn ? 'pointer' : 'default', color: 'var(--text-3)', borderRight: i < 3 ? '1px solid var(--border)' : 'none' }}>{l}</div>
            ))}
          </div>
          ...
          <div style={{ position: 'absolute', bottom: 14, left: 14, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, overflow: 'hidden', zIndex: 30 }}>
            {['Main Floor', 'Basement'].map((f, i) => (
              <div key={f} style={{ fontFamily: 'var(--font-mono)', fontSize: 9, padding: '5px 11px', cursor: 'pointer', color: i === 0 ? 'var(--blue)' : 'var(--text-3)', textTransform: 'uppercase', borderBottom: i === 0 ? '1px solid var(--border)' : 'none', background: i === 0 ? 'var(--blue-dim)' : 'transparent' }}>{f}</div>
            ))}
          </div>
        </div>
        ...
        {/* RIGHT PANEL                                                     */}
        {/* ============================================================== */}
        <div style={{ width: 306, background: 'var(--surface-1)', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
          {/* Panel mode selector */}
          <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--surface-1)', flexShrink: 0 }}>
            {(['room', 'budget', 'team'] as const).map(t => (
              <div key={t} onClick={() => setRightTab(t)} style={{ flex: 1, fontFamily: 'var(--font-mono)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '.06em', padding: '7px 0', textAlign: 'center', cursor: 'pointer', color: rightTab === t ? 'var(--text)' : 'var(--text-3)', borderBottom: rightTab === t ? '2px solid var(--blue)' : '2px solid transparent', position: 'relative', top: 1, transition: 'color .15s' }}>{t.charAt(0).toUpperCase() + t.slice(1)}</div>
            ))}
          </div>
          ...
          {rightTab === 'room' && (<>
            <div style={{ padding: '12px 14px 10px', borderBottom: '1px solid var(--border)', background: 'var(--surface-2)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                <div style={{ fontFamily: 'var(--font-cond)', fontSize: 18, fontWeight: 800, letterSpacing: '.06em', flex: 1, color: 'var(--text)' }}>{r.name}</div>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: SCH[r.st], flexShrink: 0 }} />
              </div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-3)', marginBottom: 8 }}>{r.dim}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-2)' }}>{r.prog}%</div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
                {([['Tasks', r.tasks, 'var(--blue)'], ['Hours', r.hrs, 'var(--text)'], ['Flags', String(r.flags), r.flags > 0 ? 'var(--amber)' : 'var(--green)']] as const).map(([l, v, c]) => (
                  <div key={l} style={{ background: 'var(--surface-3)', border: '1px solid var(--border)', borderRadius: 4, padding: '5px 8px' }}>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 7, textTransform: 'uppercase', letterSpacing: '.07em', color: 'var(--text-3)', marginBottom: 2 }}>{l}</div>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              {['tasks', 'comments', 'photos'].map(t => (
                <div key={t} onClick={() => setTab(t)} style={{ flex: 1, fontFamily: 'var(--font-mono)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '.06em', padding: '8px 0', textAlign: 'center', cursor: 'pointer', color: tab === t ? 'var(--text)' : 'var(--text-3)', borderBottom: tab === t ? '2px solid var(--blue)' : '2px solid transparent', position: 'relative', top: 1, transition: 'color .15s' }}>{t.charAt(0).toUpperCase() + t.slice(1)}</div>
              ))}
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
              {tab === 'tasks' && (
                <div style={{ padding: '10px 12px' }}>
                  {Object.entries(groups).map(([tr, tasks]) => (
                    <div key={tr}>
                      <div style={{ fontFamily: 'var(--font-cond)', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', padding: '8px 0 4px' }}>{TRM[tr] ?? tr}</div>
                      {tasks.map((t, i) => {
                        const done = t.s === 'done', act = t.s === 'active', blk = t.s === 'blocked';
                        const tc = TRC[t.tr] ?? TRC.trim;
                        return (
                          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '7px 9px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, marginBottom: 4, opacity: done ? .6 : 1 }}>
                            <div style={{ width: 15, height: 15, borderRadius: 3, border: `1.5px solid ${done ? '#22C55E' : act ? '#1A56DB' : 'var(--text-3)'}`, background: done ? '#22C55E' : act ? 'rgba(26,86,219,.15)' : 'var(--surface-3)', flexShrink: 0, marginTop: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: done ? '#fff' : '' }}>{done ? '\u2713' : blk ? '!' : ''}</div>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: 12, fontWeight: 500, lineHeight: 1.3, marginBottom: 2, color: 'var(--text)' }}>{t.n}</div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                ...
                                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)' }}>{t.h}</span>
                              </div>
                            </div>
                            <div style={{
                              ...
                              background: done ? 'var(--green-dim)' : act ? 'var(--blue-dim)' : blk ? 'var(--red-dim)' : 'var(--green-dim)',
              ...
              {tab === 'comments' && (
                <div style={{ padding: '10px 12px' }}>
                  {r.cm.length === 0 ? (
                    <div style={{ padding: '24px 10px', textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-3)' }}>No comments yet</div>
                  ) : r.cm.map((c, i) => {
                    const key = `${cur}-${i}`;
                    ...
                    return (
                      <div key={key} style={{ marginTop: 10, opacity: res[key] ? .4 : 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text)' }}>{c.a}</span>
                          ...
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)', marginLeft: 'auto' }}>{c.t}</span>
                        </div>
                        <div style={{ fontSize: 11, color: 'var(--text-2)', lineHeight: 1.55, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, padding: '7px 9px', marginLeft: 26 }}>
                          {c.b}
                          {c.ph && <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 6, fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--blue)', background: 'var(--blue-dim)', border: '1px solid rgba(26,86,219,.2)', padding: '3px 7px', borderRadius: 3, cursor: 'pointer' }}>{c.ph}</div>}
                        </div>
                        {!res[key] && <div onClick={() => setRes(p => ({ ...p, [key]: true }))} style={{ marginLeft: 26, marginTop: 4, fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)', cursor: 'pointer' }}>\u2713 Mark resolved</div>}
                      </div>
                    );
                  })}
                </div>
              ...
              {tab === 'photos' && (
                <div style={{ padding: '10px 12px' }}>
                  {([['Before', ['Overview', 'Window', 'Sub-floor']], ['During', ['Demo', '+', '+']]] as const).map(([lbl, photos]) => (
                    <div key={lbl}>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.08em', margin: '10px 0 6px' }}>{lbl}</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, marginBottom: 8 }}>
                        {photos.map((p, i) => (
                          <div key={i} style={{ aspectRatio: '1', borderRadius: 4, border: `1px ${p === '+' ? 'dashed' : 'solid'} var(--border)`, background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: p === '+' ? 18 : 11, cursor: 'pointer', color: 'var(--text-3)' }}>
            ...
            <div style={{ padding: '10px 12px', borderTop: '1px solid var(--border)', background: 'var(--surface-2)', flexShrink: 0 }}>
              <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
                {['Note', 'Flag', 'Block'].map(t => (
                  <div key={t} style={{ fontFamily: 'var(--font-mono)', fontSize: 8, padding: '3px 7px', borderRadius: 3, border: '1px solid var(--border)', cursor: 'pointer', color: 'var(--text-3)', background: 'var(--surface-3)', textTransform: 'uppercase' }}>{t}</div>
                ))}
              </div>
              <textarea value={note} onChange={e => setNote(e.target.value)} placeholder={`Add a note for ${r.name}\u2026`} rows={2}
                style={{ width: '100%', background: 'var(--surface-3)', border: '1px solid var(--border)', borderRadius: 4, padding: '7px 9px', color: 'var(--text)', fontFamily: 'var(--font-sans)', fontSize: 11, resize: 'none', outline: 'none' }} />
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, marginTop: 6 }}>
                <div onClick={() => setNote('')} style={{ fontFamily: 'var(--font-mono)', fontSize: 9, padding: '5px 11px', borderRadius: 3, background: 'var(--surface-3)', color: 'var(--text-3)', cursor: 'pointer', textTransform: 'uppercase' }}>Cancel</div>
          ...
          {rightTab === 'budget' && (
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', marginBottom: 8 }}>Financials</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, marginBottom: 12 }}>
                {([['Quoted', `$${FINANCIALS.quoted.toLocaleString()}`, 'var(--blue)'], ['Invoiced', `$${FINANCIALS.invoiced.toLocaleString()}`, 'var(--text)'], ['Paid', `$${FINANCIALS.paid.toLocaleString()}`, 'var(--green)'], ['Remaining', `$${FINANCIALS.remaining.toLocaleString()}`, 'var(--amber)']] as const).map(([l, v, c]) => (
                  <div key={l} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 4, padding: '6px 8px' }}>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 7, textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 2 }}>{l}</div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 500, color: c }}>{v}</div>
                  </div>
                ))}
              </div>

              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', marginBottom: 8 }}>Cost Breakdown</div>
              <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, padding: '8px 10px', marginBottom: 12 }}>
                {([['Labour', FINANCIALS.labourQuoted, '#14B8A6'], ['Materials', FINANCIALS.materialsQuoted, '#A855F7'], ['Overhead', FINANCIALS.overhead, 'var(--text-3)'], ['Change Orders', FINANCIALS.changeOrders, 'var(--amber)']] as const).map(([l, v, c], i, arr) => (
                  <div key={l} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 0', borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    ...
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text)' }}>${v.toLocaleString()}</span>
                  </div>
                ))}
              </div>

              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', marginBottom: 8 }}>Labour Hours</div>
              <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, padding: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  <div style={{ position: 'relative', width: 44, height: 44 }}>
                    <ProgressRing pct={Math.round((FINANCIALS.actualHours / FINANCIALS.budgetedHours) * 100)} color="#14B8A6" size={44} stroke={3} />
                    <span style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', fontFamily: 'var(--font-mono)', fontSize: 9, color: '#14B8A6' }}>{Math.round((FINANCIALS.actualHours / FINANCIALS.budgetedHours) * 100)}%</span>
                  </div>
                  <div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{FINANCIALS.actualHours}h / {FINANCIALS.budgetedHours}h</div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)' }}>{FINANCIALS.budgetedHours - FINANCIALS.actualHours}h remaining</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 4 }}>
                  {([['Budgeted', FINANCIALS.budgetedHours, 'var(--text-3)'], ['Scheduled', FINANCIALS.scheduledHours, 'var(--blue)'], ['Actual', FINANCIALS.actualHours, '#14B8A6']] as const).map(([l, v, c]) => (
                    <div key={l} style={{ flex: 1, background: 'var(--surface-3)', borderRadius: 3, padding: '4px 6px' }}>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 7, textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 1 }}>{l}</div>
                ...
                <div style={{ marginTop: 8, height: 4, background: 'var(--surface-3)', borderRadius: 2, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', height: '100%', background: '#14B8A6', borderRadius: 2, width: `${(FINANCIALS.actualHours / FINANCIALS.budgetedHours) * 100}%` }} />
                </div>
              </div>
            </div>
          )}
          ...
          {rightTab === 'team' && (
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', marginBottom: 8 }}>Crew on Site</div>
              {TEAM.map(m => (
                <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, marginBottom: 4 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-3)' }}>{m.role}</div>
                  </div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-2)', background: 'var(--surface-3)', padding: '3px 8px', borderRadius: 3 }}>{m.hours}</div>
                </div>
              ))}

              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', marginTop: 16, marginBottom: 8 }}>Room Status</div>
                ...
                {Object.entries(rooms).map(([id, rm]) => (
                  <div key={id} onClick={() => { setCur(id); setRightTab('room'); }} style={{ background: 'var(--surface-2)', border: `1px solid ${id === cur ? SCH[rm.st] + '44' : 'var(--border)'}`, borderRadius: 4, padding: '6px 8px', cursor: 'pointer' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 3 }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-cond)', fontSize: 10, fontWeight: 700, letterSpacing: '.04em', color: 'var(--text)' }}>{rm.name}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--text-3)' }}>{rm.prog}%</span>
                    </div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 7, color: 'var(--text-3)', marginTop: 2 }}>{rm.tasks} tasks \u00b7 {rm.hrs}</div>
                  </div>
                ))}
              </div>

              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-3)', marginTop: 16, marginBottom: 8 }}>Quick Actions</div>
              {['Log Time', 'Add Photo', 'Create Change Order', 'Message Homeowner'].map(a => (
                <div key={a} style={{ padding: '8px 10px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, marginBottom: 3, fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  {a}<span style={{ color: 'var(--text-3)' }}>\u203a</span>
                </div>
              ))}
            </div>
          )}
