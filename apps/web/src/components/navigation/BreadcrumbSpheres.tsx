    ...
    <div className="flex items-center gap-2 py-4 overflow-x-auto">
      {items.map((item, index) => (
        <div key={item.id} className="flex items-center gap-2">
          ...
          <Link
            href={item.href}
            className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--surface)] hover:bg-[var(--surface-2)] transition-colors"
          >
            <span className="w-6 h-6 rounded-full bg-primary-500 text-white text-xs flex items-center justify-center">
              {item.score}
            </span>
            <span className="text-sm text-[var(--mid)] whitespace-nowrap">{item.label}</span>
          </Link>
        </div>
      ))}
    </div>
