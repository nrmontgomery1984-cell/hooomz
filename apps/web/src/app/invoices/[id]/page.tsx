...
export default function InvoiceDetailPage() {
  ...
  if (isLoading) {
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        <p style={{ fontSize: 13, color: 'var(--text-3)' }}>Loading invoice…</p>
      </div>
    );
  }
  ...
  if (!invoice) {
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        <p style={{ fontSize: 13, color: 'var(--text-3)' }}>Invoice not found</p>
        <button
          onClick={() => router.back()}
          style={{ marginTop: 12, fontSize: 12, color: 'var(--teal)', background: 'none', border: 'none', cursor: 'pointer' }}
        >
          Go back
        </button>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '16px 16px 60px' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', marginTop: 2 }}>{formatDate(invoice.dueDate)}</div>
          </div>
          <div>
            <div style={labelStyle}>Total</div>
            <div style={{ fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-mono)', color: 'var(--text)', marginTop: 2 }}>{formatCurrency(invoice.totalAmount)}</div>
          </div>
          <div>
            ...
            <div style={{
              ...
              {invoice.lineItems.map((li, i) => (
                ...
                >
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', color: '#6B7280', textAlign: 'right' }}>{li.quantity}</span>
                  <span style={{ fontFamily: 'var(--font-mono)', color: '#6B7280', textAlign: 'right' }}>{formatCurrency(li.unitCost)}</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, color: '#111827', textAlign: 'right' }}>{formatCurrency(li.totalCost)}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {payments.map((p: PaymentRecord, i: number) => (
                <div
                  ...
                  style={{
                    ...
                    gap: 10,
                    padding: '8px 12px',
                    borderBottom: i < payments.length - 1 ? '1px solid #F3F4F6' : undefined,
                    fontSize: 11,
                  }}
                >
                  <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, color: '#059669', width: 80 }}>
                  ...
                  <span style={{ color: '#9CA3AF', fontSize: 10 }}>{formatDate(p.date)}</span>
                  {p.reference && (
                    <span style={{ marginLeft: 'auto', fontSize: 9, color: '#9CA3AF' }}>#{p.reference}</span>
                  )}
                </div>
              ))}
            </div>
        ...
        {invoice.notes && (
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: '#6B7280', margin: 0 }}>{invoice.notes}</p>
          </div>
        )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <button
                ...
                style={{
                  ...
                  letterSpacing: '0.04em',
                  color: '#FFFFFF',
                  background: confirmCancel ? '#EF4444' : 'none',
                  borderRadius: 8,
                  border: confirmCancel ? '1px solid #EF4444' : '1px solid #FCA5A5',
                  cursor: 'pointer',
                  padding: '0 16px',
                  ...(confirmCancel ? {} : { color: '#EF4444' }),
                }}
              >
