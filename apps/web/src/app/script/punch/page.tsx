...
const CURRENT_PHASE: ScriptPhase = 'punch';
...
const PUNCH_CHECKLIST_TEMPLATE: ChecklistItem[] = [
  ...
  { id: 7, type: 'normal', label: 'Lock P phase with performance colour → advance to Turnover', note: '🟢 Zero deficiencies + sign-off · 🟡 Minor items outstanding · 🔴 Significant rework', checked: false },
];
...
export default function PunchPage() {
  ...
  const [checklist, setChecklist] = useState<ChecklistItem[]>(
    PUNCH_CHECKLIST_TEMPLATE.map((c) => ({ ...c })),
  );
  ...
  const handleSelectProject = useCallback((id: string) => {
    ...
    setChecklist(PUNCH_CHECKLIST_TEMPLATE.map((c) => ({ ...c })));
  }, []);
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#FDFCFA' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          Punch · {filteredProjects.length} Active
        </div>
