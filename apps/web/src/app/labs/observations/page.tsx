    ...
    <div style={{ minHeight: '100vh', paddingBottom: 96, background: 'var(--bg)' }}>
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        <div className="max-w-lg mx-auto px-4 py-4">
          ...
          <h1 className="text-xl font-bold" style={{ color: 'var(--charcoal)' }}>Observations</h1>
          <p className="text-xs" style={{ color: 'var(--muted)' }}>{observations.length} field observations</p>
        </div>
      ...
      <div className="max-w-lg mx-auto px-4 mt-4 space-y-3">
        {isLoading ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm" style={{ color: 'var(--muted)' }}>Loading observations...</p>
          </div>
        ) : sorted.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-sm" style={{ color: 'var(--muted)' }}>No observations yet</p>
            <p className="text-xs mt-1" style={{ color: 'var(--muted)' }}>Observations are captured as crew members work on projects</p>
          </div>
        ) : (
          sorted.map((obs) => (
