...
Merged Working Document  |  v2.0  |  February 2026
Nathan Montgomery  •  Red Seal Journeyman Carpenter  •  Moncton, NB
Sources: Opus v1.0 + Claude Code v1.0 + conversation history
...
New leads appear as cards in a pipeline: New → Contacted → Site Visit Booked → Estimate Sent → Won/Lost. Each card shows: name, lead source badge, scope tags, budget range, instant estimate range, lead temperature indicator, days since created. One-tap actions: Call, Text, Schedule Site Visit.
Where Labs Fits
LABS INTEGRATION
...
Step 4: Scope Per Room (Contextual Questions)
For each selected room (or batch), the system asks only the questions relevant to the trades indicated. If they said "floors and paint," they don't see tile or drywall questions. Questions flow as a guided sequence with smart recommendations already visible:
...
"What do you want?" → System suggests 2–3 products based on design preferences + budget. Product cards with photo, name, price/sqft, Labs rating.
...
System suggests colors based on preference profile: "You said you like warm tones—here are popular warm neutrals."
...
Loop Generation
The approved scope auto-generates the project's Loop structure—nested containers organized by phase, each containing tasks derived from SOPs. Three-axis filtering applies:
...
Referral System
Each completed client gets a unique referral link. Referrals that convert earn the referrer a credit (discount on future work or maintenance). Referral source is tracked in the Activity Log on the new lead, connecting the two client records.
Callback Handling
...
App Experience
The Client Portal transitions from "active project" to "your home" mode permanently. The project summary lives in their profile. Sections: before/after gallery, warranty tracker with expiration reminders, maintenance schedule and tips, "Start a New Project" button (one-tap repeat business), Hooomz Profile link (shareable—valuable at home sale), maintenance subscription upsell (gentle, context-relevant), referral section with unique link and shareable card designed for text/social.
...
Phase 1 (Home Shows): Lead magnet. Homeowner scans room with ArUco markers, selects materials, sees VR preview. Generates lead with design preference signals (dwell time per product, final selections).
Phase 3 (Proposal): Conversion tool. Homeowner sees their actual space with proposed products applied. "See your new floors before we install them."
...
Closeout
ProfileEntry
property_id, project_id, scope_summary, products_installed, photos, dates, warranties
Closeout
FollowUpSequence
project_id, events[], last_sent, next_due
Follow-up
Referral
referrer_id, referee_lead_id, status, credit_amount
Follow-up
TradePartnerPayment
...
Captures property, preferences, budget; pre-fills everything downstream; running total updates live
...
End of Document
Hooomz Interiors • Job Flow & Product Vision • Merged v2.0 • February 2026
