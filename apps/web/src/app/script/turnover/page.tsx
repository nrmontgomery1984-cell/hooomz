...
const CURRENT_PHASE: ScriptPhase = 'turnover';
...
const TURNOVER_CHECKLIST_TEMPLATE: ChecklistItem[] = [
  { id: 1, type: 'normal', label: 'Full site cleanup — all debris, tools, and protection removed', note: '', checked: false },
  { id: 2, type: 'normal', label: 'Final photos taken — completed work documented', note: 'Minimum 3 per room — match pre-work photos', checked: false },
  { id: 3, type: 'killer', label: 'Client final walkthrough completed', note: '⚑ Walk every room, confirm satisfaction before leaving site', checked: false },
  ...
  { id: 6, type: 'killer', label: 'Client signature on completion — job formally closed', note: '⚑ No exceptions', checked: false },
  { id: 7, type: 'auto', label: 'Job record archived — SCRIPT complete', note: '⚡ Automated — triggers Passport completion notification to client', checked: false },
];
...
export default function TurnoverPage() {
  ...
  const [checklist, setChecklist] = useState<ChecklistItem[]>(
    TURNOVER_CHECKLIST_TEMPLATE.map((c) => ({ ...c })),
  );
  ...
  const handleSelectProject = useCallback((id: string) => {
    ...
    setChecklist(TURNOVER_CHECKLIST_TEMPLATE.map((c) => ({ ...c })));
  }, []);
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#FDFCFA' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          Turnover · {filteredProjects.length} Active
        </div>
