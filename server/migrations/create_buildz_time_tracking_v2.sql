-- =====================================================
-- HOOOMZ BUILDZ - TIME TRACKING SYSTEM (UPDATED)
-- Comprehensive time tracking for construction projects
-- Works with existing phases table structure
-- =====================================================

-- =====================================================
-- 1. UPDATE EXISTING PHASES TABLE
-- Add global Buildz construction phases if they don't exist
-- =====================================================

-- Insert Buildz-specific global phases (using existing table structure)
INSERT INTO phases (name, description, is_global_template, display_order) VALUES
  ('Pre-Construction', 'Planning and preparation before work begins', TRUE, 5),
  ('Demo', 'Demolition and removal work', TRUE, 15),
  ('Closing In', 'Enclosing and sealing the structure', TRUE, 35),
  ('Finishing', 'Final touches and finishing work', TRUE, 40),
  ('Closeout', 'Final inspection and project completion', TRUE, 65)
ON CONFLICT ON CONSTRAINT unique_phase_name DO NOTHING;

-- Update existing phases to ensure they're marked as global templates
UPDATE phases
SET is_global_template = TRUE
WHERE name IN ('Estimate', 'Rough-In', 'Installation', 'Finish', 'Inspection', 'Warranty')
