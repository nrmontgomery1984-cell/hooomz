...
import { useRiskEntriesBySop, useUpdateRiskEntry } from '@/lib/hooks/useRiskRegister';
import type {
  SOPProcedure,
  SOPCriticalStandard,
...
function StatPill({ label, value, color }: { label: string; value: string | number; color?: string }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 18, fontWeight: 700, color: color ?? 'var(--charcoal)' }}>{value}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
              {steps.map((step, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--mid)', lineHeight: 1.5 }}>
                    {step}
                  </span>
                </div>
              ))}
              ...
              <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {checkpoints.map((cp: ProcedureCheckpoint, i: number) => (
                    <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td style={{ padding: '6px 8px', fontWeight: 500, color: 'var(--charcoal)' }}>{cp.checkpoint}</td>
              ...
              <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {tests.map((t: ProcedureTest, i: number) => (
                    <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td style={{ padding: '6px 8px', fontWeight: 500, color: 'var(--charcoal)' }}>{t.test}</td>
...
function RiskFlagBanner({ sopId }: { sopId: string }) {
  const updateMutation = useUpdateRiskEntry();
  const { data: riskEntries = [] } = useRiskEntriesBySop(sopId);

  const flaggedEntries = riskEntries.filter((e) => e.sopFlaggedForReview);
  ...
  return (
    <div
      className="max-w-lg md:max-w-full mx-auto px-4 md:px-6"
      style={{ paddingTop: 12 }}
    >
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <span style={{ fontSize: 13, color: 'var(--charcoal)' }}>
            This SOP has been flagged for review based on{' '}
            <strong style={{ color: 'var(--amber)' }}>{flaggedEntries.length}</strong>{' '}
            logged risk event{flaggedEntries.length !== 1 ? 's' : ''}.
          </span>
        </div>
...
export default function SOPDetailPage() {
  ...
  const flaggedRiskCount = riskEntries.filter((e) => e.sopFlaggedForReview).length;
  ...
  const trade = TRADE_STYLES[sop.trade] ?? TRADE_STYLES.default;
        ...
        <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
          <div className="max-w-lg md:max-w-full mx-auto px-4 md:px-6 py-3 md:py-4">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, color: 'var(--muted)' }}>v{sop.version}</span>
            </div>

            {/* Title */}
            <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--charcoal)', lineHeight: 1.3 }}>
              {sop.title}
            </h1>
          ...
          <div className="max-w-lg md:max-w-full mx-auto px-4 md:px-6 pb-3">
            <div style={{
              ...
              <StatPill label="Stop" value={stopConditions.length} color={stopConditions.length > 0 ? 'var(--red)' : undefined} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {sop.learningObjectives.map((obj, i) => (
                  <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    ...
                    <span style={{ fontSize: 13, color: 'var(--mid)', lineHeight: 1.5 }}>{obj}</span>
                  </div>
                ))}
              </div>
            </div>
